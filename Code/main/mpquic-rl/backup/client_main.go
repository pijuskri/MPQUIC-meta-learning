...
//"application/octet-stream",type 4  are all font
...
var (
	host = "10.1.0.1:6121"
	//host  = "192.168.0.109:6121"
	objs  []*Obj
	count []byte
	bow   *browser.Browser
)

func parse(graphPath string, browser string) map[string]*http2.PriorityParam {
	...
	if browser == "Safari" {
		for _, obj := range objs {
			if strings.Contains(obj.Download.Type, "html") {
				obj.PriorityParam = &http2.PriorityParam{Weight: 255}
			} else if strings.Contains(obj.Download.Type, "javascript") || strings.Contains(obj.Download.Type, "js") || strings.Contains(obj.Download.Type, "css") {
				obj.PriorityParam = &http2.PriorityParam{Weight: 24}

			} else if strings.Contains(obj.Download.Type, "image") {
				obj.PriorityParam = &http2.PriorityParam{Weight: 8}
			} else if strings.Contains(obj.Download.Type, "octet-stream") || strings.Contains(obj.Download.Type, "4") { //font, type = 4
				obj.PriorityParam = &http2.PriorityParam{Weight: 16}
			} else {
				obj.PriorityParam = &http2.PriorityParam{Weight: 16}
			}
		...
		for _, obj := range objs {
			if strings.Contains(obj.Download.Type, "html") || strings.Contains(obj.Download.Type, "css") || strings.Contains(obj.Download.Type, "octet-stream") || strings.Contains(obj.Download.Type, "4") {
				obj.PriorityParam = &http2.PriorityParam{Weight: 255}
	...
	{
		for _, obj := range objs {
			if strings.Contains(obj.Download.Type, "html") || strings.Contains(obj.Download.Type, "css") {
				obj.PriorityParam = &http2.PriorityParam{Weight: 32}
			} else if strings.Contains(obj.Download.Type, "javascript") || strings.Contains(obj.Download.Type, "js") {
				obj.PriorityParam = &http2.PriorityParam{Weight: 32}

			} else if strings.Contains(obj.Download.Type, "image") {
				obj.PriorityParam = &http2.PriorityParam{Weight: 22}
			} else if strings.Contains(obj.Download.Type, "4") || strings.Contains(obj.Download.Type, "octet-stream") { //fonts
				obj.PriorityParam = &http2.PriorityParam{Weight: 42}
			} else {
				obj.PriorityParam = &http2.PriorityParam{Weight: 2}
			}
			priorityURL["https://"+host+obj.Path] = obj.PriorityParam
...
// download one object by go routine (remove dependency)
func shiSafariGet(obj *Obj, wg *sync.WaitGroup) {
	obj.Download.Lock.Lock()
	if obj.Download.Started {
		obj.Download.Lock.Unlock()
		return
...
// download one object by go routine (remove dependency)
func shiFirefoxGet(obj *Obj, wg *sync.WaitGroup, wgAll *sync.WaitGroup) {
	obj.Download.Lock.Lock()
	if obj.Download.Started {
		obj.Download.Lock.Unlock()
		return
...
// download one object until fully loaded (remove dependency)
func shiChromeGet(obj *Obj) {

	pth := "https://" + host + obj.Path
	obj.Download.Lock.Lock()
	obj.Download.Started = true
	obj.Download.StartTime = time.Now()
	obj.Download.Lock.Unlock()
	...
	utils.Infof("Finish downloading of %s", pth)
	obj.Download.Lock.Lock()
	obj.Download.Complete = true
	obj.Download.CompleteTime = time.Now()
	obj.Download.Lock.Unlock()
...
// initiate concurrently of all the objects
func safariGet() {
	utils.Infof("Get by Safari browser")
	var wg sync.WaitGroup

	for _, obj := range objs {
		utils.Infof("obj type: " + obj.Download.Type)
		wg.Add(1)
		shiSafariGet(obj, &wg) //SHI
	}
	wg.Wait()

}
func fireFoxGet() {
	utils.Infof("Get by Firefox browser")

	var wg sync.WaitGroup
	var wgAll sync.WaitGroup

	//css,js leader; html img fonts icon follower; others unblocked
	for _, obj := range objs {

		if strings.Contains(obj.Download.Type, "css") || strings.Contains(obj.Download.Type, "js") || strings.Contains(obj.Download.Type, "javascript") {
			wg.Add(1)
			wgAll.Add(1)
			shiFirefoxGet(obj, &wg, &wgAll) //SHI

		} else if !(strings.Contains(obj.Download.Type, "html") || strings.Contains(obj.Download.Type, "4") || strings.Contains(obj.Download.Type, "octet-stream") || strings.Contains(obj.Download.Type, "image")) {
			wgAll.Add(1)
			shiFirefoxGet(obj, &wg, &wgAll) //SHI
		}
	...
	for _, obj := range objs {
		if strings.Contains(obj.Download.Type, "html") || strings.Contains(obj.Download.Type, "4") || strings.Contains(obj.Download.Type, "octet-stream") || strings.Contains(obj.Download.Type, "image") {
			wgAll.Add(1)
			shiFirefoxGet(obj, &wg, &wgAll) //SHI
		}
	}
...
func chromeGet() {
	utils.Infof("Get by Chrome browser")

	// highest bucket: HTML, CSS, fonts  256
	for _, obj := range objs {
		if strings.Contains(obj.Download.Type, "html") || strings.Contains(obj.Download.Type, "css") || strings.Contains(obj.Download.Type, "4") || strings.Contains(obj.Download.Type, "octet-stream") {
			shiChromeGet(obj) //SHI
		}
	}

	// high bucket: all JS  220
	for _, obj := range objs {
		if strings.Contains(obj.Download.Type, "js") || strings.Contains(obj.Download.Type, "javascript") {
			shiChromeGet(obj) //SHI
		}
	}
	// low bucket: img 147
	for _, obj := range objs {
		if strings.Contains(obj.Download.Type, "image") {
			shiChromeGet(obj) //SHI
		}
	}
	...
	// lowest: others 110
	for _, obj := range objs {
		if !strings.Contains(obj.Download.Type, "js") && !strings.Contains(obj.Download.Type, "javascript") && !strings.Contains(obj.Download.Type, "image") &&
			!strings.Contains(obj.Download.Type, "html") && !strings.Contains(obj.Download.Type, "css") && !strings.Contains(obj.Download.Type, "4") && !strings.Contains(obj.Download.Type, "octet-stream") {
			shiChromeGet(obj) //SHI
		}
	}
...
func main() {
	...
	flag.String("www", "/var/www", "www data")
	flag.String("certpath", "", "certificate directory")
	pathScheduler := flag.String("ps", "", "path scheduler")
	// streamScheduler = flag.String("ss", "RoundRobin", "stream scheduler")
	browser := flag.String("b", "Safari", "browser type")
	...
	// add priority info for each stream
	priorityURL := parse(graphPath[0], *browser)
	...
	// request objects according to the browser's dependency tree
	if *browser == "Chrome" {
		chromeGet()
	} else if *browser == "Firefox" {
		fireFoxGet()
