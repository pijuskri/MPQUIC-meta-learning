...
#ffmpeg -y -i $VIDEO -c:a aac -ac 2 -ab 128k -c:v libx264 -x264opts keyint=96:min-keyint=96:no-scenecut -b:v 6000k -maxrate 12000k -bufsize 24000k -s 2560x1440 -vf scale=2560:1440 -filter:v fps=30 video_1440_30fps.mp4
...
#ffmpeg -y -i $VIDEO -c:a aac -ac 2 -ab 128k -c:v libx264 -x264opts keyint=96:min-keyint=96:no-scenecut -b:v 15000k -maxrate 30000k -bufsize 60000k -s 3840x2160 -vf scale=3840:2160 -filter:v fps=30 video_2160_30fps.mp4
